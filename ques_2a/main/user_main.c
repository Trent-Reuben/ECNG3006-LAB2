...
Question 2(a): Round-robin scheduling where tasks have the same priority
*/
...
//This program make use of one defined priority due to round-robin (one priority level) hence the following are defined
#define PRIORITY_EQUAL   1
...
void app_main(void)
{
    ...
    //start the tasks
    xTaskCreate(gpio_on, "gpio_on", 2042, NULL, PRIORITY_EQUAL, NULL);
    xTaskCreate(status_msg, "status_msg", 2042, NULL, PRIORITY_EQUAL, NULL);
    xTaskCreate(gpio_off, "gpio_off", 2042, NULL, PRIORITY_EQUAL, NULL);
